...
@section('form-content')
    <div class="background-image-form height-main-form p-3 h-100">
        <section class="container ">
            <div class="col-12 d-flex flex-wrap mb-4">
                ...
                <div class="col-4 p-1 position-relative">
                    <div class="delete position-absolute">
                        <form
                        action="{{route('user.image.destroy', $image)}}"
                        method="POST"
                        class="image-destroyer"
                        onclick="return confirm('Sei sicuro di voler eliminare la seguente foto?')">
                            @csrf
                            @method('DELETE')
                            <button
                            type="submit"
                            class="rounded-1 delete">
                                &#10006;
                            </button>
                        </form>
                    </div>
            ...
            <form
            class="text-center bg-light rounded p-5 height-main-form "
            ...
            enctype="multipart/form-data">
                ...
                <div class="form-group ">
                    ...
                    <textarea rows="1.5" class="form-control  @error('description') is-invalid @enderror" name="description" id="description" placeholder="* campo richiesto" value="{{ old('description', $apartment->description) }}" required autocomplete="on"
                        ...
                        <div class="col-sm-10 text-left h-50 overflow-auto">
                            @foreach($services as $service)
                                <div class="form-check form-check-inline">
                                    <input
                                    class="form-check-input @error('service') is-invalid @enderror"
                                    type="checkbox"
                                    id="service-{{ $service->id }}" value="{{ $service->id }}" name="services[]"
                                    @if(in_array($service->id, old('services', $service_id))) checked @endif>
                                    <label
                                    class="form-check-label" for="service-{{ $service->id }}">
                                        {{$service->name}}
                                    </label>
                                </div>
                            @endforeach
